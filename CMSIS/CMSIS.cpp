...
//#define DO_Q31_RADIX2 need to fix compilation issues first!!!
  ...
  for (int i = 0; i < 1024 * multK; i++) {
    double x = i * T;
    src_q15[i*2] = (q15_t)(f2q15(0.5*sin(50.0 * 2.0 * PI * x) + 0.5*sin(80.0 * 2.0 * PI * x)));
    src_q15[i*2+1] = 0; // set the imaginary part to 0
  }
 
  arm_cfft_radix2_init_q15(&s_q15, 1024 * multK, 0, 1); // Initialize the CFFT instance for 8-point FFT
  arm_cfft_radix2_q15(&s_q15, src_q15); 
  ...
  for (int i = 0; i < 1024 * multK; i++) {
    double x = i * T;
    src_q31[i*2] = (q31_t)f2q31(0.5 * sin(50.0 * 2.0 * PI * x) + 0.5 * sin(80.0 * 2.0 * PI * x));
    src_q31[i * 2 + 1] = 0;
  }
