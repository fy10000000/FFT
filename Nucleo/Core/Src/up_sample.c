...
static inline double sinc_pi(double x) {
  ...
  return arm_sin_f32(M_PI * x) / (M_PI * x);//sin(M_PI * x) / (M_PI * x);
}
...
static inline double blackman_sym(int n, int half) {
  ...
  return a0 - a1 * arm_cos_f32(phi) + a2 * arm_cos_f32(2.0 * phi);//a0 - a1 * cos(phi) + a2 * cos(2.0 * phi);
}
  ...
  c32* out, size_t Nout_cap)
{
  ...
  c32 y[1024];// = (c32*)malloc(Nout * sizeof(c32));
  //if (!y) {
  //	  return 0;
  //}
  ...
  */
  // reuse y to get through this
  iq_resamp_forward(&st_back, yr, Nout, y, Nout);
